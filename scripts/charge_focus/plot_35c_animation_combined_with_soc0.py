"""Variant of plot_35c_animation_combined.py that extends the window to
include Step 3 (0% rest) and Step 4 (0->20% charge), so the resulting GIF
covers SOC 0 -> 80 instead of 20 -> 80.

Builds its own cache in reports/longrun_cycling_35c_charge_focus/_cache_with_soc0/
and writes the GIF to .../06_animation/longrun_35c_combined_with_soc0.gif so
the original 20->80 cache + GIF are untouched.
"""
...
CACHE_DIR = OUT_ROOT / "_cache_with_soc0"
CACHE_DIR.mkdir(parents=True, exist_ok=True)
OUT_DIR = OUT_ROOT / "06_animation"
OUT_DIR.mkdir(parents=True, exist_ok=True)
OUT_PATH = OUT_DIR / "longrun_35c_combined_with_soc0.gif"
...
# Extended SEGMENTS: prepend Step 3 (0% rest) and Step 4 (0->20 charge)
SEGMENTS = [
...
def _build_cache(rebuild: bool = False) -> tuple[dict, pd.DataFrame, np.ndarray, np.ndarray, np.ndarray]:
    ...
    print("building extended cache (Steps 3-11) ...")
...
def main():
    ...
    def update(i):
        ...
        suptitle.set_text(
            f"35°C charge-focus (with SOC 0%) · r{int(run_idx[i]):03d} · t={t_h[i]:5.2f} h   "
    ...
    print("rendering combined-with-soc0 GIF ...")
