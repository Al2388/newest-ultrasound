...
---------------------
  ...
  POST /api/jog_z               Adjust transducer Z height  (key required)
...
def _hardware_free():
    """
    FastAPI dependency — raises HTTP 409 if the TiePie HS5 is already in use.

    Both CScanService and AScanService drive the same physical oscilloscope.
    This guard prevents them from being started concurrently and fighting over
    the hardware, which would cause libtiepie errors or corrupt data.
    """
    if scanner.running:
...
class JogRequest(BaseModel):
    """Relative Z displacement for adjusting the transducer coupling height."""
    z: float = Field(..., ge=-20.0, le=20.0,
                     description="Z displacement in mm (positive = up)")
...
class AScanConfig(BaseModel):
    ...
    session_name:  str   = Field("",
                                 description="Label included in the output filename")
...
app     = FastAPI(title="Ultrasound Battery Lab", version="2.3")
...
async def dashboard(request: Request):
    ...
    The API key is injected into the HTML template server-side as a Jinja2
    variable so it never appears in client-side source code or HTTP parameters.
    """
...
@app.post("/api/jog_z", dependencies=[Depends(_require_key)])
async def jog_z(req: JogRequest):
    """
    Move the Z axis by req.z mm relative to the current position.

    Used to adjust the water-coupling gap between the transducer face and the
    battery surface. Positive values move the transducer upward (away from battery).
    """
    success, msg = scanner.jog_z_axis(req.z)
    return {"success": success, "msg": msg}
...
async def download_latest_scan():
    ...
    """
    out_dir = scanner.config["out_dir"]
    files   = sorted(glob.glob(os.path.join(out_dir, "scan_*.npz")))
    if not files:
...
async def get_latest_meta():
    ...
    """
    out_dir = scanner.config["out_dir"]
    files   = sorted(glob.glob(os.path.join(out_dir, "scan_*_meta.json")))
    if not files:
        return {}
...
async def ascan_download():
    ...
    if ascan._h5_path and os.path.exists(ascan._h5_path):
        ...
        files = sorted(glob.glob(os.path.join(ascan.config["out_dir"], "ascan_*.h5")))
        if not files:
